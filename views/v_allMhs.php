...
<?php foreach ($list_mhs->result() as $rowMhs) { ?>
	<div class="modal fade" id="modal_<?php echo $rowMhs->user_id; ?>" tabindex="-1" role="dialog" aria-labelledby="confirmModalLabel" aria-hidden="true">
	      ...
	      <div class="modal-body">
	        <div class="input-group mb-3">
			  <div class="input-group-prepend">
			    ...
			    <h6>Apakah anda yakin akan menghapus <?php echo strtoupper($rowMhs->user_name); ?>?</h6>
			  </div>
			</div>
	      </div>
	      <div class="modal-footer">
	        <form action="<?php echo base_url();?>index.php/adminChangeUser/delete/<?php echo $rowMhs->user_id; ?>" method="POST">
	            <div class="form-group">
	               <input type="hidden" value="<?php echo "$rowMhs->user_id"; ?>" class="form-control" name="user_id">
...
<div class="container mt-5 mb-0 p-5 rounded">
	<h3 class="py-3" style="text-align: center;">Tabel Data Mahasiswa</h3>
	<?php 
	  	...
	  	<div class="container">
	  		...
	  		<table class="table table-bordered table-success table-hover table-striped">
	  			<thead class="thead-dark">
	  				...
	  				<th class="bg-info align-middle" scope="col">Kelas</th>
	  				<th class="bg-info align-middle" scope="col">Program Studi</th>
	  				<th colspan="2" class="bg-info align-middle" scope="col">
	  					<a class="btn btn-success btn-hadir" href="<?php echo base_url();?>index.php/adminChangeUser/insert/Mahasiswa">
	  						<i class="fa fa-plus-circle"></i> | Tambah Data
	  					</a>
	  				</th>
	  			</thead>
	  		<?php $no = 1; foreach ($list_mhs->result() as $rowMhs) { ?>
	  			<?php if ($rowMhs->user_status == 'Mahasiswa') { ?>
	  			<tr>
	  				<td><?php echo $no; ?></td>
	  				<td><?php echo $rowMhs->user_name; ?></td>
	  				<td><?php echo $rowMhs->user_nim; ?></td>
	  				<td><?php echo $rowMhs->user_address; ?></td>
	  				<td><?php echo $rowMhs->user_gender; ?></td>
	  				<?php $tgl_lahir = date("d M Y", strtotime($rowMhs->user_date)); ?>
	  				<td><?php echo $tgl_lahir; ?></td>
	  				<td><?php echo $rowMhs->class_name; ?></td>
	  				<td><?php echo $rowMhs->jurusan_name; ?></td>
	  				<td>
	  					<a class="btn btn-info btn-usai" href="<?php echo base_url();?>index.php/adminChangeUser/update/<?php echo $rowMhs->user_id; ?>"><i class="fa fa-pencil"></i> Edit</a>
	  				</td>
	  				<td>
	  					<a class = "btn btn-primary" href="" data-toggle='modal' data-target = <?php echo "'#modal_".$rowMhs->user_id."'"; ?>><i class="fa fa-trash-o"></i> Hapus</a>
	  				</td>
	  			</tr>
	  			<?php $no++; } elseif ($rowMhs->user_status == NULL) { ?>
	  			<tr>
	  				<td>-</td>
